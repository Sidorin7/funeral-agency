const burger = () => {
  ...
  const navigation = document.querySelector(".header__top");
  const overlay = document.querySelector(".overlay");
  const isMobileMenu = () => window.matchMedia("(max-width: 1500px)").matches;

  const closeMenu = () => {
    navigation.classList.remove("header__top--open");
    overlay.classList.remove("overlay--show");
    menuBtn.classList.remove("burger--active");
    navigation
      .querySelectorAll(".is-submenu-open")
      .forEach((li) => li.classList.remove("is-submenu-open"));
  };
